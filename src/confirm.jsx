...
export const Cart=()=>{
    ...
    const{shopped,getTotal,getTotalPrice} = useContext(ShopContext);
let totalItems=getTotal();
let totalPrices=getTotalPrice();


const AddedList =(props)=>{
    
    return(
        <div style={{padding:'2px' ,maxHeight:'15%',maxWidth:'100%',display:'flex',justifyContent:'space-between',}}>
         <div style={{height:'15%',maxWidth:'20%',display:'flex'}}><img style={{objectFit:'cover'}} src={props.pic}/> </div>
          <div>
          <div> <b>{props.name}</b> </div>
          <div style={{display:'flex',justifyContent:'space-evenly'}}>
            <div style={{color:'orangered'}}>{props.number}X</div>
            <div style={{color:'gray'}}> ${props.price}</div>
            <div >  <div> total: ${props.price*props.number}</div></div>
            </div>
            </div>
        </div>
    )
...
ShowList();
    return(
        <div className='cart'>
           <div className='YourCart'> Your Cart({totalItems})</div>
           
               {(!totalPrices==0)?
               AddedItems.map(AddedItem=>{
                   if(AddedItem.num>0){
                       return(<div>
                           <AddedList pic={AddedItem.item.image.desktop} name={AddedItem.item.name} number={AddedItem.num} price={AddedItem.item.price} id={AddedItem.id}/>
                       </div>)
                   }
               })
          ...
          <div className='total-price'><div> Total Price: </div> <div>$ <b>{totalPrices}</b> </div></div>
        </div>
    )
}
...
export default function Modal({open,onClose}){
    ...
    return ReactDom.createPortal(
        ...
        <Cart/>
            <button onClick={onClose} style={{color:'orangered'}}>Confirm Order</button>
