...
export const Cart=()=>{
    ...
    const{shopped,getTotal,getTotalPrice,removeFromCart,setDefault} = useContext(ShopContext);
    ...
    return(
        <div className='AddedItems'>
          <div><b>{props.name}</b></div>
            ...
            <div >  <div> <b> total: ${props.price*props.number}</b></div></div>
           <div className='btn'> 
...
const[isOpen,setIsOpen]=useState(false);
    return(
        <div className='cart'>
           <div className='YourCart'> Your Cart({totalItems})</div>
               ...
               AddedItems.map(AddedItem=>{
                   if(AddedItem.num>0){
                       return(<div>
                           <AddedList name={AddedItem.item.name} number={AddedItem.num} price={AddedItem.item.price} id={AddedItem.id}/>
                       </div>)
                   }
               })
        ...
        <button style={{color:'white',backgroundColor:'orangered',borderRadius:'20px'}} onClick={()=>setIsOpen(true)}>Confirm Order</button>
